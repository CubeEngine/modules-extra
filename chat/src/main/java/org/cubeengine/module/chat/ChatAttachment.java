...
public class ChatAttachment extends UserAttachment
{
    ...
    {
        // TODO async stuffs
        muted = fromNullable(dsl().selectFrom(TableMuted.TABLE_MUTED).where(TableMuted.TABLE_MUTED.ID.eq(
    ...
    {
        return muted.transform(m -> m.getValue(TableMuted.TABLE_MUTED.MUTED)).orNull();
    }
    ...
    public void setMuted(Date muted)
    {
        ...
        if (!this.muted.isPresent())
        {
            this.muted = Optional.of(dsl().newRecord(TableMuted.TABLE_MUTED).newMuted(getHolder()));
        }
        this.muted.get().setValue(TableMuted.TABLE_MUTED.MUTED, muted);
