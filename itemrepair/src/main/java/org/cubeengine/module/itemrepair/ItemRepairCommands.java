...
public class ItemRepairCommands extends ContainerCommand
{
    ...
    public void add(Player context)
    {
        ...
        {
            i18n.send(context, NEGATIVE, "You are already adding a repair block!");
            return;
        }
        if (this.removeRequests.contains(context.getUniqueId()))
        {
            i18n.send(context, NEGATIVE, "You are already removing a repair block!");
            return;
        }
        this.addRequests.add(context.getUniqueId());
        i18n.send(context, NEUTRAL, "Rightclick the block.");
    }
